...
def relabel_bridge_actions(traj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    movement_actions = traj["observation"]["state"][:6] - \
        traj["observation"]["state"][:6]
...
def bridge_oxe_dataset_transform(trajectory):
    ...
    for i in range(0, len(trajectory)-1):
        ...
        trajectory[i]["action"][:6] = trajectory[i+1]["observation"]["state"][:6] - \
            trajectory[i]["observation"]["state"][:6]  # Get realtive change action
...
def maniskill_dataset_transform(trajectory):
    for i in range(0, len(trajectory)):
        ...
        # TODO: Not sure if this is the information for wrist pos and rotation
        trajectory[i]["observation"]["eef_state"] = trajectory[i]["observation"]["state"][1:7]
...
def libero_dataset_transform(trajectory):
    for i in range(0, len(trajectory)):
        ...
        trajectory[i]["observation"]["eef_state"] = trajectory[i]["observation"]["state"][:6]
        trajectory[i]["action"] = trajectory[i]["action"].numpy()
        trajectory[i]["action"][-1:] = 1 - \
            np.clip(trajectory[i]["action"][-1:], 0, 1)
        trajectory[i]['observation']["natural_language_instruction"] = trajectory[i]["language_instruction"]
    return trajectory
...
def saytap_transform(trajectory):
    for i in range(0, len(trajectory)):
        ...
        # No gripper state in SayTap
        trajectory[i]["observation"]["gripper_state"] = 0
        trajectory[i]["action"] = trajectory[i]["action"].numpy()
...
def apply_transforms(episode, cfg, dataset_name):
    ...
    # Example transformation: resize images, normalize actions, etc.
    episode = TRANSFORMS[cfg.dataset.dataset_indicies[dataset_name]["dataset_key"]](
        episode)
    return episode
...
class CircularBuffer:
    ...
    def __init__(self, size, cfg, model):
        ...
        if self._cfg.dataset.encode_with_t5:
            self._tokenizer = T5Tokenizer.from_pretrained(
                self._cfg.dataset.t5_version)
            self._text_model = T5ForConditionalGeneration.from_pretrained(
                self._cfg.dataset.t5_version)
        ...
        if self._cfg.dataset.num_episodes > 0:
            for dataset_name in self._cfg.dataset.dataset_indicies:
                self._builders[dataset_name] = tfds.builder_from_directory(
                    builder_dir=dataset_name)
                print(
                    "dataset size:", self._builders[dataset_name].info.splits["train"].num_examples)
        ...
        # text encoder to tokens:
        self._encode_txt = lambda s: [stoi[c] for c in s]
        # token decoder to text:
        self._decode_txy = lambda l: ''.join([itos[i] for i in l])
        ...
        if self._cfg.dataset.load_dataset is True:
            ...
            dataset = datasets.load_dataset(self._cfg.dataset.to_name, split='train[:{}]'.format(
                self._cfg.dataset.buffer_size), keep_in_memory=True)
            print("Time to load huggingface dataset:", time.time() - start_)
            dataset_tmp = {
                # Some loading optimizations to improve debugging
            ...
            print("Time to load huggingface data and copy: ",
                  time.time() - start__)
            for i in range(len(dataset_tmp["img"])):
                if len(dataset_tmp["action"][i:i+self._cfg.policy.action_stacking]) < self._cfg.policy.action_stacking:
                    print("Skipping index", i, "because action length is less than",
                          self._cfg.policy.action_stacking)
                    continue
                ...
                self.add(
                    ...
                    language_instruction=dataset_tmp["t5_language_embedding"][i] if cfg.dataset.encode_with_t5 and (
                        "t5_language_embedding" in dataset_tmp) else None,
                    terminal=0,
                    pose=pose,
                )
    ...
    def update_internal_dataset(self, size, old_data=None):
        ...
        _dataset_tmp = {
            ...
            # This is a list of strings, not a tensor
            "goal_text_full": ["" for _ in range(size)],
    ...
    def print_mem_footprint(self):
        ...
        print("Memory used by the dataset cBuffer:",
              get_total_dict_size(self._dataset_tmp) / 1e6, "MB")
        # Compute the memory use of the datset part self._dataset_tmp["img"]
        print("Memory used by the dataset cBuffer image:",
              asizeof.asizeof(self._dataset_tmp["img"]) / 1e6, "MB")
        print("Memory used by the dataset cBuffer goal image:",
              asizeof.asizeof(self._dataset_tmp["goal_img"]) / 1e6, "MB")
        print("Memory used by the dataset cBuffer: t5_language_embedding", asizeof.asizeof(
            self._dataset_tmp["t5_language_embedding"]) / 1e6, "MB")
    ...
    def add(self, obs, action, goal, goal_img, language_instruction=None, pose=None, terminal=0,
        ...
        self._dataset_tmp["img"][self._index] = torch.tensor(
            np.array(obs), dtype=torch.uint8, device=self._cfg.device)
        self._dataset_tmp["goal_img"][self._index] = torch.tensor(
            np.array(goal_img), dtype=torch.uint8, device=self._cfg.device)
        self._dataset_tmp["action"][self._index] = torch.tensor(
            action, dtype=torch.float, device=self._cfg.device)
        if pose is not None:
            self._dataset_tmp["pose"][self._index] = torch.tensor(
                pose, dtype=torch.float32, device=self._cfg.device)  # Store robot pose

        # Store the full goal text
        self._dataset_tmp["goal_text_full"][self._index] = goal
        self._dataset_tmp["terminal"][self._index] = torch.tensor(
            terminal, dtype=torch.uint8, device=self._cfg.device)
        ...
        if self._cfg.dataset.encode_with_t5:
            if language_instruction is not None:
                ...
                self._dataset_tmp["t5_language_embedding"][self._index] = torch.tensor(
                    language_instruction[:self._cfg.max_block_size], dtype=torch.float, device=self._cfg.device)
            else:
                print("Encoding goal text with T5:", goal)
                with torch.profiler.record_function("Process goal text with T5"):
                    goal_ = self._model.process_text_embedding_for_buffer(
                        goal, tokenizer=self._tokenizer, text_model=self._text_model)
                    self._dataset_tmp["t5_language_embedding"][self._index] = torch.tensor(
                        goal_, dtype=torch.float, device=self._cfg.device)

        goal_ = " " * self._cfg.max_block_size
        goal_ = goal[:self._cfg.max_block_size] + \
            goal_[len(goal):self._cfg.max_block_size]
        # assert len(goal_) == self._cfg.max_block_size
        self._dataset_tmp["goal"][self._index] = torch.tensor(
            self._encode_txt(goal_), dtype=torch.float, device=self._cfg.device)
        self._count += 1
        if self._cfg.dataset.download_all is True and (self._count >= self._size):
            # Double the buffer sizes to fit all the data.
            print("Doubling buffer size from", self._size, "to",
                  self._size * 2, " to fit incomming data.")
            self.print_mem_footprint()
            old_data = self._dataset_tmp
            self._dataset_tmp = self.update_internal_dataset(
                self._size * 2, old_data=old_data)
        else:
    ...
    def get_batch_grp(self, split, cfg, batch_size, morphology=0):
        ...
        if self._cfg.policy.use_image_augmentations and split == 'train':
            # TODO:
            # Add image Augmentations to improve performance
            transform_crop_scale = v2.Compose([
                v2.RandomResizedCrop(size=(cfg.image_shape[0], cfg.image_shape[1]),
                                     scale=(0.9, 1.0), ratio=(0.95, 1.05)),
                v2.ColorJitter(brightness=0.2, contrast=0.2,
                               saturation=0.1, hue=0.05),
                v2.RandomAdjustSharpness(sharpness_factor=1.5, p=0.3),
                v2.GaussianBlur(kernel_size=3, sigma=(0.1, 0.5)),
                v2.ToDtype(torch.float32),  # Convert to float [0,1]
            ])
        else:
            transform_crop_scale = v2.Compose([
                # Convert to float [0,1] after crop/resize
        ...
        ix = np.random.randint(min(self._count, self._size)-(
            (cfg.policy.action_stacking + cfg.policy.obs_stacking)-1), size=(batch_size,))

        # Convert to [B, T, C, H, W] format for torchvision transforms, and back.
        obs_ = data["img"][ix-cfg.policy.obs_stacking +
                           1].to(torch.float).unsqueeze(1)
        for i in range(1, cfg.policy.obs_stacking):  # This is slow but works.
            # concatenate along the time dimension
            obs_ = torch.cat(
                (obs_, data["img"][ix-cfg.policy.obs_stacking+1+i].unsqueeze(1)), axis=1)
        # Convert to [B, T, C, H, W] format for torchvision transforms, and back.
        obs_ = transform_crop_scale(obs_)
        # Rearranging the image to have the stacked history in the last channel dimension)  # Flatten the time dimension for batching
        x = self._model.normalize_state(
            rearrange(obs_, 'b t h w c -> b h w (c t)', c=3, t=cfg.policy.obs_stacking))

        pose = data["pose"][ix].to(torch.float32).unsqueeze(
            1)  # Convert to [B, T, C]

        if cfg.dataset.encode_with_t5:
            x_goal = torch.tensor(
                data["t5_language_embedding"][ix], dtype=torch.float, device=cfg.device)
        else:
            x_goal = data["goal"][ix]

        x_goal_img = self._model.normalize_state(transform_crop_scale(
            data["goal_img"][ix].to(torch.float)))  # [B, C, H,  W]
        # Convert to [B, H, W, C] format from torchvision.
        x_goal_img = x_goal_img

        # TODO:
        # Provide the block masking logic for the attention head
        y = self._model.encode_action(
            # discrete or continuous actions
            data["action"][ix + cfg.policy.obs_stacking - 1])
        if cfg.policy.action_stacking > 1:
            ...
            for i in range(1, cfg.policy.action_stacking):
                # stack on time timension.
                y = torch.cat((y, self._model.encode_action(
                    data["action"][ix + i])), axis=1)
    ...
    def save(self, path):
        ...
        self._dataset_tmp = {k: v for k, v in self._dataset_tmp.items() if not (
            k == "t5_language_embedding" and self._cfg.dataset.encode_with_t5 is False)}
        for key in self._dataset_tmp:
            ...
            if isinstance(self._dataset_tmp[key], list):  # Text data
                ...
                self._dataset_tmp[key] = self._dataset_tmp[key][:self._count].detach(
                ).cpu().numpy()
        ...
        a_std, a_mean = (self._dataset_tmp["action"]).std(
            axis=0) + 1e-8, self._dataset_tmp["action"].mean(axis=0)
        self._cfg.dataset.action_std = [float(x) for x in a_std]
        self._cfg.dataset.action_mean = [float(x) for x in a_mean]

        with open('./config.json', 'w') as f:
            json.dump(OmegaConf.to_container(
                self._cfg, resolve=True), f, indent=2)
...
def get_dataset_portion(builder, cbuffer, cfg, list_, dataset_name=None):
    ...
    for c in list(list_):
        # Most likely a very slow way to get data from the dataset, but it is a better mix
        datasetRemote = builder.as_dataset(
            split='train[' + str(c) + ':' + str(c+cfg.dataset.chunk_size) + ']')
        gc.collect()
        for episode in datasetRemote:
            ...
            goal_img = cv2.resize(np.array(
                episode[-1]['observation']["image"], dtype=np.float32), (cfg.image_shape[0], cfg.image_shape[1]))
            for i in range(len(episode)):  # Resize images to reduce computation
                obs = cv2.resize(np.array(
                    episode[i]['observation']["image"], dtype=np.float32), (cfg.image_shape[0], cfg.image_shape[1]))
                ...
                cbuffer.add(obs=obs,
                            ...
                            goal=episode[i]['observation']["natural_language_instruction"].numpy(
                            ).decode(),
                            goal_img=goal_img,
                            terminal=1 if (i == (len(episode) - 1)
                                           ) or episode[i]['is_terminal'] else 0,
                            pose=pose,
...
def get_multi_dataset_portion(builders, cbuffer, cfg):
    ...
    for dataset_name, builder in builders.items():
        ...
        print(" size_ ", builder.info.splits["train"].num_examples, " total samples to fetch", int(samples_),
        ...
        if cfg.dataset.download_all is True:
            ...
            ix = np.random.randint(
                builder.info.splits["train"].num_examples-cfg.dataset.chunk_size, size=int(samples_))

        if use_generator:
            # Use from_generator for memory-efficient loading
            print(f"Using from_generator approach for {dataset_name}")
            def gen_func(): return _create_dataset_generator(builder, cfg, dataset_name, ix)
            ...
            get_dataset_portion(builder, cbuffer, cfg,
                                dataset_name=dataset_name, list_=ix)
...
def my_main(cfg: DictConfig):
    ...
    # Use CPU for dataset creation to avoid GPU memory issues.
    cfg.device = "cpu"
